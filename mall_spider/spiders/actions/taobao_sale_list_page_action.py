...
from mall_spider.common.constants import SpiderParams, SpiderUrls
...
class TaobaoSaleListPageAction(HttpAction):
    ...
    def __build_data(self, context):
        ...
        data = SpiderParams.build_taobao_search_data(query, page, 'list', 'nav,selecthot,onesearch', '_sale', **extra)
    ...
    def do_execute(self, context):
        ...
        sale_url = SpiderUrls.get_taobao_serarch_url(type=TaobaoSearchType.sale, timestamps=timestamps,
                                                     sign=sign, data=data)
        sale_http_request = HttpRequest(url=sale_url, method=HttpMethod.GET)
        context.attach(Context.KEY_TAOBAO_SALE_LIST_HTTP_REQUEST, sale_http_request)
    ...
    def unmarshal(self, context, response):
        ...
        if result.find('login.m.taobao.com') != -1:
            raise CookieExpiredException('cookie is expired,msg:taobao sale list page')
        if result.find('FAIL_SYS_TOKEN_EMPTY') != -1:
            raise CookieNeedUpdateException('cookies need update,msg:taobao sale list page')
